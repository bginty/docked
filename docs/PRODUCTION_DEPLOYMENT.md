...
- Authenticated Admin inspection verified AUD, Australia/Melbourne, metric and kilograms. The supplied correspondence address and `support@docked.com.au` were configured. After the owner's mailbox-setup report, Notifications was rechecked: the sender textbox still contained `support@docked.com.au`, Shopify still reported **Unverified**, and **Resend verification** remained available. New customer accounts are active with sign-in links enabled; the signed-out entry dialog rendered, but authentication, logout and order history remain untested. Australia is the only active market.
