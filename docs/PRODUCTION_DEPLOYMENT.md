...
- `Docked Production Candidate 2026-08`, ID `130871427130`, was created and fully pushed as an **unpublished** theme. The permanent preview URL is `https://cfbexf-h4.myshopify.com?preview_theme_id=130871427130` and the editor URL is `https://cfbexf-h4.myshopify.com/admin/themes/130871427130/editor`.
- The storefront password was entered only in the authenticated in-app Browser, allowing the unpublished candidate to render without removing password protection. Manual responsive QA ran at 320, 360, 375, 390, 768, 1024 and 1440 px. A real 320 px horizontal-overflow defect was found, repaired by constraining the mobile wordmark to `11rem`, re-pushed to theme `130871427130`, and retested with no overflow. Core home, collection, cart, contact and 404 routes plus the earlier mobile-menu mechanism, search, predictive search, cart drawer and desktop navigation passed within this limited browser run. The later six-link Main menu rendered on desktop but was not reverified in the mobile menu. Full multi-browser, accessibility, Lighthouse, product, checkout and delivery testing remains incomplete, so rendered QA is blocked.
- Authenticated Admin inspection verified AUD, Australia/Melbourne, metric and kilograms. The supplied correspondence address and `support@docked.com.au` were configured; Shopify still reports the sender email as unverified and a verification email was sent. New customer accounts are active with sign-in links enabled; the signed-out entry dialog rendered, but authentication, logout and order history remain untested. Australia is the only active market.
...
- The Main menu is now `Home`, `Shop`, `Contact`, `How It Works`, `Safety and Care`, `FAQ`; those entries rendered and were verified on the password-protected candidate desktop. The mobile-menu mechanism passed earlier, but the updated six-link contents were not reverified on mobile. The existing Footer menu is now `Search`, `Contact`, `How It Works`, `Safety and Care`, `FAQ`, `Track Your Order`, `Accessibility`.
- Three additional named Admin resources were created: Footer shopping menu ID `198327042106` with the seven planned collections; Footer support menu ID `198327074874`, duplicated from the support footer; and Footer legal menu ID `198327107642` with only Shopify's existing native Privacy Policy resource. That resource and the separate hidden custom Privacy Policy Page are both unapproved. Unapproved Terms were intentionally withheld. Local source maps **Explore** to `footer-shopping-menu` and **Help and policies** to `footer-support-menu`, but that source change is not yet evidenced as re-pushed or rendered on Shopify; do not treat the named footer resources as candidate-rendered until a clean push and retest are recorded.
...
That initial full command completed successfully by 07:14 AEST on 14 August 2026 with no returned warning or error. A later CSS-only re-push delivered the repaired 320 px wordmark rule, but its exact timestamp was not retained. The next exact release-commit full push must be recorded separately; do not treat 07:14 as the timestamp of the candidate's final state. Do not paste an access token into this command or a tracked file.
...
- the seven created empty collections and reviewed navigation. The updated Main menu is candidate-rendered on desktop; its six-link contents still require mobile-menu verification, while the local named-footer mapping still requires a clean theme re-push and rendered verification; and
