...
| Preview QA observation window/operator | 14 August 2026 AEST. Timestamped screenshot artefacts span 08:41–10:11 AEST; later same-morning Admin, menu and account observations did not retain per-action timestamps / Codex using the authenticated in-app Browser |
...
| Mobile menu | Earlier mechanism pass; updated contents not reverified on mobile | The mobile menu opened and navigated during the earlier run. The later six-link Main menu rendered on desktop, but those updated contents were not re-tested in the mobile menu |
| Footer navigation | Admin configured; new source mapping not yet rendered | Existing Footer menu has `Search`, `Contact`, `How It Works`, `Safety and Care`, `FAQ`, `Track Your Order`, `Accessibility`. Three named footer menus exist, but the local `footer-shopping-menu` / `footer-support-menu` section mapping has not yet been re-pushed and rendered |
...
| Responsive overflow | Pass after repair | Real 320 px defect fixed with `11rem` mobile wordmark and retested; other required widths manually passed |
...
| Sender-domain authentication | Not run; Shopify sender remains **Unverified** | Delivered-message headers and provider configuration review |
